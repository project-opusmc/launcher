...
int main(int argc, char *argv[]) {
    ...
    if (java_index >= argc) {
        fputs("[RBW/STUB] Java executable argument is missing\n", stderr);
        return 64;
    }

    const char *working_directory = getenv("RBW_GAME_WORKDIR");
    if (working_directory != NULL && working_directory[0] != '\0'
        && chdir(working_directory) != 0) {
        fprintf(
            stderr,
            "[RBW/STUB] could not use game directory %s: %s\n",
            working_directory,
            strerror(errno));
        return 73;
    }
    ...
    fprintf(stderr, "[RBW/STUB] could not start Java: %s\n", strerror(errno));
    return 127;
}
