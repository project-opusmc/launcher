...
1. **No unverified bytes reach the game.** Every managed artifact — version
   ...
   files, Log4j config, RBW coremod, and RBW client mod — must match its declared size and
...
10. **OptiFine is local-import only.** RBW accepts only the exact locked local
...
11. **No token in the webview.** The frontend receives `accountStored: bool` and
    a redacted `"Name (uuid)"` profile string. Never a token, refresh
    credential, or client secret.
...
13. **Only the refresh token is persisted, and only in the OS keychain.** Service
    `dev.rbw.client.microsoft.refresh-token`. No file-backed token cache.
...
19. **Every supported product launch uses the Forge stdin bridge.** The desktop
    ...
    legacy direct branch also refuses authenticated identities defensively.
...
21. **Mojang's Log4j `${...}` deny filter must be present.** Exactly one
    ...
    fails closed. RBW adds a second filter denying `Session ID is token:`.
...
27. **QA, UI Preview, and Premium roots cannot overlap.** `discover_qa` rejects
    ...
    root rejects both existing editions. `RBW_HOME` does not affect QA or
    Preview; `RBW_QA_HOME` and `RBW_UI_PREVIEW_HOME` must be absolute.
28. **QA compiles no account code.** Not disabled at runtime — absent. QA
    registers no `login_with_microsoft`, `logout`, or keychain access.
...
32. **Forge owns the game classloader.** The supported launch enters
    LaunchWrapper and its `LaunchClassLoader`; RBW transforms game classes only
...
33. **The RBW Forge artifacts are exact.** Before staging, the coremod checksum,
    ...
    Forge discovers both only from RBW's isolated game directory, never from an
...
43. **The macOS outcome comes from the status file, not `open`'s exit code.**
    The foreground app stub immediately `exec`s Java, so RBW must not use
    `open -W`: LaunchServices can falsely report that its tracked process
    disappeared. RBW starts the app with `open -n` and waits for the bootstrap
...
48. **QA and UI Preview managed Forge mod sets are allowlisted.** They consist
    only of the exact imported OptiFine JAR, verified RBW coremod, and verified
    RBW client mod. Unexpected top-level managed-mod entries fail launch; a
...
49. **Offline mode is not an access-control system.** A QA player can pick any
    username; QA demos require operator-controlled servers with a whitelist. The
...
50. **The embedded Client ID must be Mojang-approved before authenticated
    ...
    launcher's identity.
51. **Unknown or unfinished product behavior is never mocked.** If RBW lacks
